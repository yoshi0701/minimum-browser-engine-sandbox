...
impl<'a> HtmlParser<'a> {
    ...
    while self.chars.peek().is_some() {
      ...
      if self.chars.peek().map_or(false, |c| *c == '<') {
        ...
        if self.chars.peek().map_or(false, |c| *c == '/') {
          ...
          let close_tag_name = self.consume_white(is_valid_tag_name);

          self.consume_white(|x| x != '>');
          self.chars.next();
  ...
  fn parse_node(&mut self) -> Node {
    let tagname = self.consume_while(is_valid_name);
  ...
  fn parse_attributes(&mut self) -> AttrMap {
    ...
    while self.chars.peek().map_or(false, |c| *c != '>') {
      ...
      let value = if self.chars.peek().map_or(false, |c| *c == '=') {
        self.consume_while(char::is_whilespace);
    ...
    F: fn(char) -> bool,
  {
    ...
    while self.chars.peek().map_or(false, |c| condition(*c)) {
        result.push(self.cahrsnext().unwrap());
    }
